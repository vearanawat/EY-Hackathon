...
#     # Initialize graph in INTERACTIVE mode (step-by-step)
#     print(f"{Colors.OKCYAN}🔄 Initializing agent system (interactive mode)...{Colors.ENDC}")
...
#             # Store the count of messages before processing
...
#                 # Get ALL new AI messages that were added during this turn
#                 new_messages = result_state["messages"][message_count_before:]
                
#                 # Display each agent's response
#                 for msg in new_messages:
#                     if isinstance(msg, AIMessage):
#                         # Add spacing between multiple agent responses
#                         print(f"{Colors.BOLD}Agent: {msg.content}{Colors.ENDC}\n")
...
#                 state["messages"] = state["messages"][:-1]  # Remove the failed message
...
"""
Interactive CLI Chatbot for Retail Agent System - FIXED VERSION
No loops, clean responses, natural conversation flow
"""
...
def print_banner():
    ...
    print(f"{Colors.HEADER}{Colors.BOLD}🛍️  RETAIL SALES AGENT - INTERACTIVE CHAT{Colors.ENDC}")
    print("=" * 70)
    print(f"{Colors.OKCYAN}Welcome! Chat naturally and I'll help you shop!{Colors.ENDC}")
    print("=" * 70)
    print(f"\n{Colors.WARNING}Commands:{Colors.ENDC}")
    print("  • 'exit' - End conversation")
    print("  • 'new' - Start new session")
    print("  • 'users' - See all profiles")
    print("  • 'switch C002' - Switch user")
    print("=" * 70 + "\n")
...
def select_user():
    ...
    while True:
        user_id = input(f"{Colors.BOLD}Select User ID (or Enter for C001): {Colors.ENDC}").strip()
    ...
    while True:
        try:
            ...
            if user_input.lower() in ['exit', 'quit', 'bye']:
                ...
                user_details = get_user_details(user_id)
                state = initialize_session(user_id)
                print(f"{Colors.OKGREEN}✅ New session for {user_details['name']}!{Colors.ENDC}\n")
                continue
            ...
            # Remember how many messages we have now
            ...
            try:
                ...
                # Get ONLY the new messages added in this turn
                new_messages = result_state["messages"][msg_count_before:]
                
                # Print only new AI messages
                for msg in new_messages:
                    if isinstance(msg, AIMessage):
                        print(f"{Colors.BOLD}Agent:{Colors.ENDC} {msg.content}\n")
...
def main():
    ...
    if not key_found:
        ...
        print("❌ ERROR: No LLM API key found!")
        print("=" * 70)
        print(f"\n{Colors.WARNING}FREE Options:{Colors.ENDC}")
        print(f"  {Colors.OKGREEN}1. Google Gemini:{Colors.ENDC}")
        print(f"     export GOOGLE_API_KEY='your-key'")
        print(f"     Get: https://makersuite.google.com/app/apikey")
        print(f"\n  {Colors.OKGREEN}2. Groq:{Colors.ENDC}")
        print(f"     export GROQ_API_KEY='your-key'")
        print(f"     Get: https://console.groq.com\n")
        sys.exit(1)
